...
func (p2p *P2pServer) messageHandler(conn *websocket.Conn) {
	for {
		...
		case MESSAGEBLOCK:
			...
			fmt.Println("MessageBlock")
			if err := json.Unmarshal([]byte(d.Data), &blocks); err != nil {
				fmt.Println(err)
				return
			}
			// x, v :=
			p2p.service.ReplaceChain(blocks)
			// fmt.Println(x, v)
...
func (p2p *P2pServer) BroadcastClearTransaction() {
	data := P2pMessageStruct{string(json.RawMessage{}), CLEARTRANSACTIONS}
	fmt.Println("Broadcast Clear", data)
	for i := range p2p.sockets {
		p2p.sockets[i].WriteJSON(data)
	}
}
...
func ConvertStructToRawMessage(data interface{}) string {
